...
Vagrant.configure(2) do |config|
  ...
  config.vm.define "nfs-server" do |nfs|
    nfs.vm.box="shekeriev/debian-11"
    nfs.vm.hostname = "nfs-server"
    nfs.vm.network "private_network", ip: "192.168.200.103"
    nfs.vm.provider :virtualbox do |vb|
      vb.memory = 2048
      vb.cpus = 2
    end
    nfs.vm.provision "shell", path: "./setup_scripts/basic_setup.sh"
    nfs.vm.provision "shell", path: "./setup_scripts/nfs_server.sh"
  end
  ...
  config.vm.define "k8s-cp" do |cp|
    ...
    cp.vm.provision "shell", path: "./setup_scripts/nfs_client.sh"
  ...
  config.vm.define "k8s-node1" do |node1|
    ...
    node1.vm.provision "shell", path: "./setup_scripts/nfs_client.sh"
  ...
  config.vm.define "k8s-node2" do |node2|
    ...
    node2.vm.provision "shell", path: "./setup_scripts/nfs_client.sh"
